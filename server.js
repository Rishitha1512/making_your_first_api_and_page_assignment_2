...
app.get('/status-info', (req, res)=> {
  ...
  if (code == 200) {
    ...
    let obj = {
        status: 201,
        message: "Created: The request has been fulfilled and has resulted in the creation of a resource."
    };
    return res.send(obj);
} else if (code == 204) {
    let obj = {
    ...
    let obj = {
        status: 403,
        message: "Forbidden: The server understands the request, but it refuses to authorize it."
    };
    return res.send(obj);
} else if (code == 404) {
    let obj = {
    ...
    let obj = {
        status: 405,
        message: "Method Not Allowed: The request method is known by the server but is not supported for the target resource."
    };
    return res.send(obj);
} else if (code == 429) {
    let obj = {
    ...
    let obj = {
        status: 503,
        message: "Service Unavailable: The server is not ready to handle the request, usually due to temporary overloading or maintenance of the server."
    };
    return res.send(obj);
} else if (code == 504) {
    let obj = {
        status: 504,
        message: "Gateway Timeout: The server, while acting as a gateway or proxy, did not get a timely response from the upstream server."
    };
    return res.send(obj);
}else{
